...
export const updateUser = (req, res) => {  
  ...
  jwt.verify(userCookie, secret, (err, cookieId) => {
    ...
    const { email, name, profilePic, city, website } = req.body;
    const q = `UPDATE users SET email="${email}", name="${name}", profilePic="${profilePic}", city="${city}", website="${website}" WHERE id="${cookieId.id}";`;
    db.query(q, (err, data) => {
      try {
        if (err) throw err;
